...
2. **If a topic/project/domain argument is provided**, search the filesystem:
   - Use **Grep** to search `summary:` lines and note body text across `Projects/` for the topic
...
|---|------------|----------|------------------|--------------------------------------|
| 1 | 2026-02-10 | session  | acme-api         | Added rate limiting middleware...     |
| 2 | 2026-02-10 | session  | acme-api         | Fixed auth token rotation...          |
...
- **0 matches:** Report vault size ("The vault has N sessions across N projects"). Suggest broadening: different spelling, related terms, or `/recall` without filters.
...
- **No arguments (`/recall`):** Synthesize directly from auto-loaded summaries. Surface any open `[ ]` action items by grepping `Projects/` for unchecked tasks (grep pattern: `- \[ \]`). Cap at 10 action items.
