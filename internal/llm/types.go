...
//
// AgenticProvider extends Provider with multi-turn tool-use. It was
// retired in f85d1a4 with the dispatch ladder that was then its only
// consumer, and restored by flowdoc-gen-source-ingestion Phase 4 to
// back the agentic vv flowdoc gen path.
...
type ToolsRequest struct {
	...
	MaxTokens     int          // per-turn response token cap; 0 means provider default (4096 for Anthropic)
