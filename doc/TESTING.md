...
**~2455 tests** across 48 test packages + **1 integration test** (32 subtests) + **21 vault-accessor integration tests**. All passing. (Direction-C iter 168 retired the wrap-bundle / QC / dispatch test inventories — see DESIGN #92; the stamp-file iter anchor adds `tools_stamp_iter_test.go` and reshapes `tools_describe_iter_state_test.go` — see DESIGN #93. The Phase 1+2 ship of `feat/hook-shape-validation` (DESIGN #96, iter 180) adds `internal/hook/validate_test.go` — the `ValidateHooks` table-driven validator covering the iter-178 regression fixture, all 5 `hookCommand` `anyOf` variants, and strict `additionalProperties: false` enforcement — plus pre-write refusal/coexistence cases across all four `Install*` paths in `internal/hook/setup_test.go`. The `fix/hook-validator-optional-fields` follow-up (DESIGN #96 amendment) extends `TestValidateHooks` with +30 subtests covering the iter-180 `vp hook` + `timeout: 30` dogfooding regression, the per-variant optional-field accept matrix (5 variants × 4–6 fields), cross-variant rejection cases, combined-optional shape, and the `additionalProperties: false` regression-lock for truly unknown fields. The eight-commit ship of `feat/mcp-surface-handshake` (DESIGN #97, iters 184–185) adds two new test packages — `internal/atomicfile` (atomic-write primitive with vault-path stamp wiring) and `internal/surface` (`MCPSurfaceVersion` + `ReadStamp`/`WriteStamp` monotonicity + `ResolveStampDir` Project/Knowledge/Templates routing + `CheckCompatible` + `EnforceFailStop`/`EnforceWarnOnly` env-overrides) — plus surface checks in `internal/check/{surface,json}_test.go`, golden-manifest verifier in `internal/mcp/tool_surface_verify_test.go`, and the `vv vault merge-driver` subcommand and auto-install helpers in `cmd/vv/vault_merge_driver{,_install}_test.go`. The seven-commit ship of `feat/vv-worktree-gc` (DESIGN #98, iters 187–188) adds three new test packages: `internal/lockfile` (promoted from `internal/index/lock.go`; primitive `Acquire`/`AcquireNonBlocking`/`Release` lifecycle + parent-dir auto-create + `ErrLocked` sentinel under contention) and `internal/worktreegc` (porcelain parser w/ `Detached`/`Bare` shapes, `resolveDefaultBranch` resolver covering master/no-origin/unset-HEAD, claudeDetector regex w/ TrimSpace, and Run-level integration covering dry-run, alive/dead/captured/uncaptured/branch-mismatch verdicts, linked-worktree-serializes, EPERM-treated-as-alive, all-`-`-lines-captured, detached/bare skipped, default-branch-master, cherry-errors-in-detail) — all built on real `gitx.InitTestRepo` + `gitx.AddWorktree` + `gitx.LockWorktree` fixtures against `t.TempDir()`. Plus `cmd/vv/worktree_test.go` (5 candidate-parents CSV edge cases per v5-L2) and `internal/mcp/tools_worktree_gc_test.go`. The original `internal/index/lock_test.go` split: primitive tests rewritten in `lockfile_test.go`; the original integration-style concurrent-Save body preserved in `internal/index/lock_concurrency_test.go` (`TestIndexConcurrentSave`) now exercising `lockfile.Acquire`. `internal/testutil/gitx` extended with `AddWorktree`, `LockWorktree`, `WriteWorktreeMarker`, `InitTestRepoWithDefault` smoke-tested in `gitx_test.go`.)
...
| `check/toolchain_test.go` | 6 | DESIGN #100 `vv check toolchain` per-binary probe: `TestCheckToolchain_StubPresent_Pass` (stub-script PATH manipulation under `t.TempDir()` + `t.Setenv("PATH", ...)`, `--version` returns 0 with version body), `_StubExit1_Warn` (stub exits 1 → Warn), `_Absent_Warn` (binary not in PATH → Warn with "install" hint), `_EmptyOutput_PassWithMessage` (binary present but `--version` empty), `_AllProbes_RealHostSmoke` (smokes go/golangci-lint/gh/make/git on the real host PATH), `_GitVersionRealBinary` (asserts the real `git --version` parses as expected). |
...
| `mcp/tools_check_toolchain_test.go` | 5 | DESIGN #100 `vv_check_toolchain` MCP tool (zero-arg constructor, mirrors `NewGetAgentDefinitionTool()`): `TestNewCheckToolchainTool_Schema` (tool definition shape: name, description, empty input schema), `_HandlerReturnsValidJSON` (response body parses as `[]{name, status, detail}`), `_HandlerStatusValues` (lowercased `pass`/`warn`/`fail` matching `internal/check/json.go`), `_HandlerNamesPrefixed` (every entry's `name` is `tool:<bin>`), `_HandlerOrderMatchesSpecs` (response order matches the canonical spec list). |
...
| `config/config_test.go` (`[wrap]` schema, retired) | 0 | All `[wrap]`-schema tests retired alongside the `WrapConfig` struct in DESIGN #104 (renderer retirement). The 11 deleted functions covered `TestLoad_PartialMapOverride`, `TestOverlay_PartialMapMerge`, `TestOverlay_WrapDefaultModel`, `TestValidate_HappyPath`, `_DefaultModelUndefined`, `_NonAnthropicProviderRejected`, `_BadTierFormat`, `_EmptyWrapSection`, `TestDefaultConfig_WrapSection`, and the `EscalationLadder` deprecation-log pair `TestLoad_EscalationLadderDeprecationLog` / `_NoEscalationLadderNoLog`. The deprecation-warning bridge is dead code now that the section it bridged is gone. BurntSushi/toml silently ignores any orphan `[wrap]` block in operators' existing config.toml. |
...
| `llm/keyresolver_test.go` | 4 | DESIGN #89 + #107 layered API key resolution: `TestResolveAPIKey_ConfigWins` (config beats env for all four providers), `_EnvFallback` (config empty → env returned), `_BothEmpty_ActionableError` (error names both `vv config set-key <provider>` and the env var, all four providers), `_UnknownProvider` (rejects with supported-set message including `grok`). Each table-driven test fans across `anthropic`/`openai`/`google`/`grok` (DESIGN #107). |
...
| `llm/provider_test.go` | 3 | `TestNewProvider_ConfigKeyForwarded` — `NewProvider(enrich, providers)` wires the config-stored key into the provider constructor, with env-fallback preserved when config is empty; table-driven across all 4 providers (DESIGN #89, #107). `TestResolveBaseURL_Precedence` — Decision C precedence rule (`providers.<P>.base_url` > `enrichment.base_url` > "") locked across all 4 providers + the empty-string-as-openai alias + both-empty-yields-empty case (DESIGN #107). `TestNewProvider_GrokRouting` — end-to-end wiring lock for the new `grok` switch case. |
| `llm/grok_test.go` (DESIGN #107) | 3 | First-class Grok provider factory: `TestNewGrok_DefaultBaseURL` (empty baseURL falls back to `GrokDefaultBaseURL = https://api.x.ai/v1`), `_OverrideBaseURL` (explicit baseURL flows through), `_RoundTrip` (full request/response cycle through an httptest server, locking the URL path `/chat/completions`, `Authorization: Bearer <key>` header, and `Response.Content` extraction from the OpenAI-shaped payload). |
| `config/config_test.go` (DESIGN #107 additions) | 2 | `TestOverlay_ProvidersGrokMerge` (overlay `[providers.grok].api_key` flows through, other providers preserved), `TestOverlay_ProviderBaseURLOverride` (5-row table: per-provider base_url merge for all 4 providers + preserves-base-when-overlay-omits invariant). `TestDefaultAPIKeyEnv` gains the `grok → XAI_API_KEY` case. |
| `cmd/vv/config_setkey_test.go` (DESIGN #107 addition) | 1 | `TestConfigSetKey_AddGrokProvider` — `vv config set-key grok <key>` produces exactly `[providers.grok]\napi_key = "..."\n`. `TestConfigSetKey_UnknownProvider` updated to require the error message lists all four supported providers. |
...
| `mcp/tools_render_wrap_text_test.go` | 1 | Deprecation-shim error message verification (DESIGN #104). Single test `TestRenderWrapText_DeprecationShimErrors` asserts the handler returns an error containing `"retired"`, `"DESIGN #104"`, and `"vv context sync"`. The 32 prior tests covering the unified context-aware renderer (kind variants, prompt golden snapshots, vault_side_narrative_seed surface, factory boundary) retired alongside the renderer in DESIGN #104. |
