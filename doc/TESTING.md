...
**1840 tests** across 41 test packages + **1 integration test** (32 subtests) + **21 vault-accessor integration tests**. All passing. (Direction-C iter 168 retired the wrap-bundle / QC / dispatch test inventories — see DESIGN #92.)
...
| `mcp/tools_describe_iter_state_test.go` | 9 | DESIGN #92 minimal iter-state record: `TestDescribeIterState_Basic`, `_DirtyVault` (`vault_has_uncommitted_writes` true when porcelain non-empty), `_PriorIterAnchorFound` (last_iter_anchor_sha resolves via heading scan), `_BranchDetection` (`git rev-parse --abbrev-ref HEAD`), `_InvalidProjectName` (validateProjectName rejection), `_NoVaultGit` (graceful empty `last_iter_anchor_sha`), `TestLastIterAnchorSha_NoMatch`, `_TargetIterZero` (boundary), `TestGitCmdRunner_Defaulted` (default runner uses real git), `TestDescribeIterStateTool_OutputJSONShape` (4-field shape lock). |
