...
**1974 tests** across 51 test packages + **1 integration test** (31 subtests) + **21 vault-accessor integration tests** + **5 wrap-dispatch integration tests**. All passing.
...
| `mcp/tools_prepare_skeleton_test.go` | 5 | Phase 3a: `TestVVPrepareWrapSkeleton_HappyPath`, `_RejectsMissingRequired`, `_RotatesAfterWrite` (log-rotate KeepN=3), `_AcceptsThreadReplace` (H2-v3 schema). DESIGN #91 add: `_RejectsInvalidProjectSlug` (validateProjectName tightening — rejects `../etc/passwd`-shaped slugs at the handler boundary). The `_RotatesAfterWrite` test uses `withSkeletonCacheDir` which (post-`8ed4a2f`) also pins `VIBE_VAULT_HOME` to the tempdir. |
...
| `wrapbundlecache/cache_test.go` | 12 | Phase 3a host-local skeleton cache (6 baseline): `TestCache_WriteAndRead_RoundTrip`, `_AtomicWrite_NoPartialFile`, `_RotateKeepN_DeletesOldest`, `_RotateKeepN_FewerThanN`, `_Read_RejectsTraversal`, `_RotateKeepN_RejectsZero`. DESIGN #91 Phase 1 (5 added): `_RotateKeepN_PerProjectIsolation` (alpha's iter-10 survives beta's iter-100/101/102 rotation), `_Write_CreatesProjectSubdir` (MkdirAll creates `<base>/<project>/` 0o700), `_LegacyMigration_RelocatesRootFiles` (flat `<base>/iter-N-skeleton.json` moved to `<base>/_legacy/`), `_LegacyMigration_Idempotent` (second call no-op), `_RotateKeepN_IgnoresLegacyDir` (rotation walks per-project, not `<base>/_legacy`). DESIGN #91 Phase 2 (1 added): `_InspectAll_ReturnsPerProjectStats` (per-project counts + bytes + iter columns; `_legacy` returned with `LegacyIterSentinel`). |
...
| `cmd/vv/stats_wrap_test.go` | 4 | Phase 4 `vv stats wrap` subcommand integration (3 baseline): `TestStatsWrap_HappyPath`, `_EmptyJsonl`, `_ReadsBothJsonlFiles`. DESIGN #91 add: `_RendersSkeletonCache` (seeds 2 projects via `wrapbundlecache.Write`, asserts both rows + counts/bytes render under the new "wrap skeleton cache" section). The 3 baseline tests were updated to assert the new section header AND the `(no skeletons cached yet)` sentinel when no files are seeded. |
