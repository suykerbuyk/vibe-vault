...
from templates.                    Run migrations (schema 0→8)
    │                                  │
    ▼                                  ▼
...
| `context` | `schema.go` | `VersionFile` TOML struct, `ReadVersion`/`WriteVersion`, `LatestSchemaVersion` const (8), `Migration` type + registry (0→1 through 7→8), `migrationsFrom()` |
